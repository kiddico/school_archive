...
//You see I'm really not sure abou his
//I'm no sure if capacity should include he zero at the end of he string or not
//if it does then... well ok. if not hen I just need to go one more iteration 
//in the for loop that is
//it is neal impossible to read this blue font in a put window.... is there no way to change the colos?
...
std::istream& operator>>(std::istream& stream,String& in){
	...
	while((stream.peek()!=' ')&&(stream.peek()!=-1)){
		//remove here... Maybe. I may have to keep it
		if(stream.peek()=='\n'){
			while(stream.peek()=='\n')
				stream.ignore();
		}//to here
...
int String::findchar(char input){
	...
	return 256;//not sure what to do with this
}
